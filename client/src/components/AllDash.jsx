...
const dispatch =useDispatch()
  ...
  const { fixedTotal} = useSelector(
    (state) => state.product
  );
  const { fixedTotalUser } = useSelector(
    (state) => state.user.user
  );
  const { fixedTotalOrder } = useSelector(
    (state) => state.order
  );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100 p-6">
      <h1 className="text-4xl font-bold text-center mb-8 text-gray-800">HiBuy Dashboard</h1>
